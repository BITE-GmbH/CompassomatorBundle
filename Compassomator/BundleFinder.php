...
class BundleFinder {
	...
	public function __construct(KernelInterface $kernel, Filesystem $filesystem, $configRbDir, $configRbName, $bundlesDir, $asseticCssRoot) {
	...
	public function findPublicBundleDirs() {
		...
		foreach ($this->kernel->getBundles() as $bundle) {
			...
			$publicDirs[$bundleName] = rtrim($this->filesystem->makePathRelative($absolutePublicDir, $this->asseticCssRoot), '/');
		}
